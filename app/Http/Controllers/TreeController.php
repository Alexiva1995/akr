...
class TreeController extends Controller
{
    ...
    public function indexNewtwork($network)
    {
        try {
            ...
            //este es tu arreglo global de usuarios
            $users = $this->getChidrens2(Auth::id(), [], 1, 'referred_id', $allNetwork);
            $title = ($network == 'direct') ? 'Directo' : ' En Red';        // recorremos el arreglo con todos los usuarios 
            foreach ($users as $user) {
                //Hacemos una consulta a la tabla para ver si tiene inversiones activas
                $inv = Inversion::where('iduser', '=', $user->id)->where('status', 1)->first();
               
                // Si tiene inversiones activas asiganmos los campos con la informacion que nos interesa
                if (!empty($inv)) {
                    $user->invertido = $inv->invertido;
                    $user->ganacia = $inv->ganacia;
                }
            }
            //Titulo
