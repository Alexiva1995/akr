        ...
        <div class="card card-dashboard " id="bg-screen-image">
            <div class="container  fondo-shadow" id="fondo-shadow">
                @if (isset($user))
                    ...
                    @else
                        <form action="{{ route('realizar-inversion') }}" method="POST" target="_blank"
                            class="d-inline">
                @endif
            ...
            <div class="card-body" id="card">
                <h2 class="text-center mt-3 mb-2 text-white" id="total-a-pagar">Total a Pagar : $ <span
                        class="range-slider__value">70</span> USD</h2>

                @if (isset($user))
                    <input type="hidden" class="w-100" name="user" id="user" value="{{ $user->id }}" />
                    <div class="custom-control custom-switch mt-1">
                        <input type="checkbox" class="custom-control-input" id="customSwitch1" name="comision"
                            value="comision">
                ...
                <div class="d-flex justify-content-center  mb-5">
                    ...
                    @endif
                    <button type="submit" class="btn text-dark float-right"
                        style="background-color:#08C5B9;font-weight:600;">PAGAR VÍA COINPAYMENTS</button>
                </div>
            </div>
